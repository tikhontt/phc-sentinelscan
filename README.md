...
Для защиты основной системы от потенциальных атак со стороны сканируемых узлов, всегда запускайте `sentinel-scan` в изолированном контейнере:
```bash
docker run --rm -v $(pwd):/app sentinel-scan
```

## ⚠️ ВАЖНО: Риски verify=False

В текущей версии проверка SSL-сертификатов отключена (verify=False). Это необходимо для сканирования внутренних ресурсов с самоподписанными сертификатами, но несет следующие риски:

    MitM (Man-in-the-Middle): Злоумышленник может перехватить трафик сканера и подменить результаты.

    Атака на сканер: Вредоносный сервер может отправить «декомпрессионную бомбу» или специфический Payload, пытаясь вызвать сбой в работе вашего инструмента.

Рекомендация: Никогда не запускайте сканер с правами root на основной хост-машине. Используйте Docker с ограничением ресурсов (--memory и --cpus).
